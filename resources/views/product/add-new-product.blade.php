...
/*function appendImageToPreview(previewContainer, hiddenInput, imageUrl) {
    // Step 1: সবকিছু lowercase করা + slash normalize
    let cleanedUrl = imageUrl.replace(/\\/g, '/');

    // Step 2: সব path থেকে public বা domain বাদ দিয়ে শুধু gallery/... রেখে দিচ্ছি
    let relativePath = cleanedUrl;
    relativePath = relativePath
        .replace(/^.*\/storage\//, 'gallery/') // যদি /storage/gallery/... থাকে
        .replace(/^.*\/public\//, '') // public/ বাদ দিচ্ছি
        .replace(/^.*\/khut-bd-admin\//, '') // সাবডোমেইন বা ফোল্ডার বাদ
        .replace(/^\/+/, ''); // বাড়তি slash সরানো

    // Step 3: Preview এর জন্য (domain-সহ না, শুধু /storage/gallery)
    let previewPath = '/storage/' + relativePath.replace(/^gallery\//, 'gallery/');
    ...
    const imgWrapper = $(`
        <div class="thumbnail-wrapper" style="position: relative; display: inline-block; margin: auto; width: 220px; vertical-align: top;">
    ...
    // Hidden input-এ শুধু relative path save
  ...
  $(document).on('click', '.gallery-image', function () {
   ...
   /*$('#useSelectedImages, #confirmGallerySelection').on('click', function () {
    ...
    const selectedImages = $('.gallery-image.selected, #galleryImages input[type="checkbox"]:checked').map(function () {
        let path = $(this).attr('data-location') || $(this).val();
        path = path.replace(/^\/+/, ''); 
