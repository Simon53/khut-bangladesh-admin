                           ...
                           <div class="form-group col-md-12 text-center p-2">
                                <div id="mainImagePreview" class="d-flex justify-content-center mt-2">
                                    @if($product->main_image)
                                        @php
                                            $baseUrl = 'https://dev.khut.shop/khut-bd-admin/public/storage/';
...
const baseUrl = '/khut-bd-admin/public/storage/';
...
function setMainImage(path) {
    ...
    path = path.replace(/^https?:\/\/[^/]+\/khut-bd-admin\/public\/storage\//, '');
...
$(document).ready(function() {
    ...
    if (currentPath) {
        const fullUrl = baseUrl + currentPath.replace(/^https?:\/\/[^/]+\/khut-bd-admin\/public\/storage\//, '');
        $('#mainImagePreview').html('<img src="' + fullUrl + '" style="max-width:300px;">');
    }
});
...
// ===== Normalize path for preview =====
   ...
   tinymce.init({
    ...
    $(document).on('click', '.add-thumb-option', function () {
        ...
        if (imgSrc && !imgSrc.startsWith('/khut-bd-admin/public/storage/')) {
            imgSrc = '/khut-bd-admin/public/storage/' + imgSrc;
        }
...
function appendImageToPreview(container, hiddenInput, imgSrc, data = {}) {
    ...
    // === Step 1: যদি full URL আসে, শুধু filename / relative path বের করা ===
    if (imagePath.startsWith('http')) {
        try {
            const url = new URL(imagePath);
            const match = url.pathname.match(/storage\/(.+)$/);
            if (match) imagePath = match[1]; // gallery/filename.jpg
        } catch (e) {
            console.warn('Invalid URL:', imagePath);
        }
    } else {
        // যদি relative path আসে
        imagePath = imagePath.replace(/^\/?khut-bd-admin\/public\/storage\//, '')
                             .replace(/^\/?storage\//, '');
    }

    // === Step 2: Preview URL ===
    const previewUrl = '/khut-bd-admin/public/storage/' + imagePath; // preview full path
...
function updateThumbnailInput() {
    ...
    $('#thumbnailPreview .thumbnail-wrapper').each(function() {
        ...
        if(imgSrc.includes('/khut-bd-admin/public/storage/')){
            imgSrc = imgSrc.replace('/khut-bd-admin/public/storage/', '');
        }
...
$(document).ready(function () {
   ...
   $(document).on('click', '.gallery-image', function () {
    const baseUrl = window.location.origin + '/khut-bd-admin/public';
    const rawPath = $(this).attr('data-location').replace(/^\/+/, ''); // remove leading slash
    ...
    if (selectedTarget === '#mainImage') {
        // ✅ main image DB তে শুধুমাত্র relative path রাখোimageUploadForm
        $('#mainImage').val(cleanPath);
        // ✅ preview absolute URL
        $('#mainImagePreview').html(`<img src="${fullUrl}" alt="Main Image" style="max-width: 300px;">`);
  ...
  $('#localThumbnailInput').on('change', function () {
   ...
   $('#imageUploadForm').on('submit', function (e) {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.url) {
                // res.url = full URL (e.g. https://dev.khut.shop/khut-bd-admin/public/storage/gallery/abc.jpg)
                const relativePath = res.url.split('/storage/')[1]; // gallery/abc.jpg
                const fullUrl = window.location.origin + '/khut-bd-admin/public/storage/' + relativePath;
...
$('#submitUpdateProductBtn').on('click', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "/khut-bd-admin/public/product/update/" + $('#productId').val(),
        data: formData,
        processData: false,
        contentType: false,
        headers: {
