...
$('#saveImage').on('click', function () {
    ...
    axios.post('/khut-bd-admin/public/uploadGallery', formData, {
    headers: {
...
function loadGallery() {
    axios.get('/khut-bd-admin/public/galleryLoadJson')
    .then(function(response){
        $('.galleryRow').empty();
...
$('#loadMoreBtn').on('click', function() {
    ...
    axios.get('/khut-bd-admin/public/galleryLoadJsonById?id=' + firstImgId)
    .then(function(response) {
        $.each(response.data, function(i, item) {
            $("<div class='col-md-2 p-2'>").html(
                `<img data-id='${item.id}' class='imgGarrelryView' src='${item.url}'>` +
...
$(document).on('click', '.deleteImg', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.post('/khut-bd-admin/public/galleryImageDelete', formData)
            .then(function(response) {
                if (response.status === 200 && response.data.status === 'success') {
