     ...
     public function upload(Request $request){
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $imageFile) {
                ...
                $image->save($path, 75); 
                GalleryTable::create([
                    'location' => 'storage/gallery/' . $filename
                ]);
            }
     ...
     public function store(Request $request){
        try {
            ...
            $product = Product::create([
               // 'slug' => Str::slug($request->name_en) . '-' . uniqid(), // slug তৈরি
        ...
        if ($request->has('thumbnail_images')) {
            ...
            $img = $thumb['image_path']; // JS থেকে আসা path
            $imagePathToSave = '';
            ...
            if (filter_var($img, FILTER_VALIDATE_URL)) {
                ...
                // local storage path হলে base URL যোগ করো
                $img = ltrim($img, '/'); 
                $imagePathToSave = asset('storage/' . $img);
            }
   ...
   public function update(Request $request, $id){
    ...
    try {
        ...
        $product->fill([
            ...
            'product_barcode' => $request->product_barcode ?? $product->product_barcode,
    ...
    // ✅ Handle thumbnails
        ...
        foreach ($thumbnails as $thumbData) {
            ...
            if (!empty($thumbData['id'])) {
                // পুরানো thumbnail → শুধু update হবে
                $thumbnail = ProductThumbnail::find($thumbData['id']);
                if ($thumbnail) {
                    $thumbnail->update([
                        'image_path'        => $thumbData['image_path'] ?? $thumbnail->image_path,
                ...
                // নতুন thumbnail → insert হবে
                $thumbnail = ProductThumbnail::create([
                    'product_id'        => $product->id,
                    'image_path'        => $thumbData['image_path'] ?? null,
            ...
            // options reset
            $thumbnail->options()->delete();
            if (!empty($thumbData['options'])) {
                foreach ($thumbData['options'] as $opt) {
                    $thumbnail->options()->create([
                       'common_size_id' => !empty($opt['common_size_id']) ? $opt['common_size_id'] : null,
                       'body_size_id'   => !empty($opt['body_size_id']) ? $opt['body_size_id'] : null,
                       'barcode'        => !empty($opt['barcode']) ? $opt['barcode'] : null,
                    ]);
                }
            }
        }
